...
`topology_discovery.py` projects the same discovery index and deterministic review queue into `web/data/topology-discovery.json`. its 1,240 nodes carry exact source/year/category observations and source rows, allowing the browser model to derive any candidate's neighbors from all 47,288 possible overlap pairs. its 100 exported edges are only the sampled/prioritized worklist. the lazy `index.exploratory_topology` manifest keeps this unreviewed source-overlap layer separate from database-backed accepted company claims. the exporter rejects stale queue inputs and verifies pinned artifact references; it does not mutate evidence, identities, or reviews.

The canonical `/` route opens the research desk with the full retained 18,542-record index across five families, then loads full detail on selection. `?view=topology` defaults to the full 1,240-candidate source field; its source, year, research tag, exact source category, identity, candidate, and neighbor state persist in the URL. The exact 47,288 overlap pairs are derived in the pure browser model from retained placements. Aggregate lines indicate co-listing volume between research groups; candidate focus reveals exact neighbors and source rows. `?view=topology&topologyLayer=reviewed` opens the four accepted company claims. The older `?view=explore` source audit loads one `web/discovery-inventory/{source}-{year}.json` partition on demand. The `?view=overview` selected company study retains its separate 2021–2024 page and SEC period series in `web/dashboard.json`; changing an inventory year does not change a financial period. Raw source rows, tagged occurrences, candidate keys, reviewed providers, selected companies, and accepted topology claims have different grains.
