...
Page partitions contain normalized plain text, never raw HTML. The public default retains up to 6,000 characters with exact source length, displayed length, and an explicit truncation flag. `--page-text-limit 0` creates a full-text local catalog; the same UI can read it from a local copy of `web/`. Full retained source bytes remain in Postgres. Decimal financial and notional values remain decimal strings in the typed partitions; chart code explicitly converts values for display.
