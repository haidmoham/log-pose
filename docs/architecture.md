...
The exploration view derives its inventory years and row counts from `web/discovery.json` artifacts. It loads one `web/discovery-inventory/{source}-{year}.json` partition on demand when the raw inventory browser opens. This range is independent of the selected company's 2021–2024 page and financial series in `web/dashboard.json`. Raw source rows, tagged occurrences, candidate keys, reviewed providers, selected companies, and accepted topology claims have different grains. The map renders only accepted claims from `market_topology`, never directory overlap.
