...
- `npm run dashboard` serves the read-only research console from saved JSON exports and the bounded market-field read handler. `web/index.html` loads `research-model.js` for validation, calculations, deterministic reviewed-graph coordinates, and URL state; `console-ui.js` for DOM and chart primitives; `data-view.js` for full cross-source search and retained-record inspection; `explore-view.js` for source audits; `discovery-topology-model.js` for field coordinates and request-version guards; `discovery-topology-view.js` for its filters, field, and inspector; `topology-view.js` for the separate reviewed claim map; `topology-webgl.js` for its optional 3D renderer; and `app.js` for state and route composition. It does not require Postgres.
