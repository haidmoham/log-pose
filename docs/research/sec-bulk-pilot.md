...
The [versioned analysis build](sec-analysis-build.json) applies `sec-annual-earliest-filed-v1` with an explicit 2025-04-01 filing cutoff. It requires USD and one annual period per company/concept/end year, prefers named standard tags, then chooses the earliest eligible annual filing for that tag. Conflicting periods or same-day facts remain unresolved. The run selected 120/120 cells; every selected row carries its period, unit, tag, value, accession, filed date, member SHA-256, and ZIP artifact version. **Forty-four cells' earliest retained filing occurred after the calendar year of the period end.** These are ex-post 2021–2024 period observations available by the analysis cutoff, not values known at each year-end. Run `DATABASE_URL=... PYTHONPATH=src .venv/bin/python scripts/build_sec_analysis.py --as-of 2025-04-01` to rebuild it. The script requires exactly one pinned ZIP artifact in the pilot database.

An independent check of the [original CrowdStrike 2024 10-K](https://www.sec.gov/Archives/edgar/data/1535527/000153552724000007/crwd-20240131.htm) confirms total revenue of USD 3,055,555,000 (the filing displays 3,055,555 in thousands), the 2024-01-31 period end, and the accession used by the selected fact. That one check validates the example, not all 120 selected values.
