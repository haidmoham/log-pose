...
The [cohort manifest](pilot-cohort.json) fixes five companies in each of four software categories before archive retrieval. Each company has four planned year-end cells. The first full run used one exact homepage URL and one late-year crawl per cell. After retrying nine index-error cells, the [saved ingestion report](ingestion-report.json) shows **68 extractable / 7 short-text / 5 no exact match** out of 80. Every year has at least 16 extractable pages: 18/20 in 2021, 16/20 in 2022, 17/20 in 2023, and 17/20 in 2024. All nine index errors recovered on retry, which shows why a single request is not a reliable coverage estimate.

The five misses mean no eligible exact-homepage index row in the selected crawl. They do not imply that the company was absent from Common Crawl, another crawl, another historical URL, or Wayback. The seven short-text bodies have fewer than 100 normalized visible characters. They are now retained with `text_status=short` and their original source bytes. The 68/80 figure is **basic text extractability for this purposive cohort and narrow query**, not a software-market coverage rate or an actionable-claim rate. The report preserves all planned cells, attempt history, capture time, content hashes, transferred bytes, and source-file versions. Regenerate it with `DATABASE_URL=... PYTHONPATH=src .venv/bin/python scripts/summarize_ingestion.py --output docs/research/ingestion-report.json`.
...
Review a fixed cross-category sample of stored pages for concrete, quoted product, buyer, workflow, or pricing claims. Diagnose the seven short-body responses. For the unresolved and poor-quality cells, test historically evidenced URL aliases and earlier eligible crawls before measuring Wayback's incremental coverage. Record extra useful cells per request and review minute. Keep the original 80-cell denominator and distinguish retrieval from useful evidence. The public Common Crawl URL index is intended for lookups, not bulk enumeration; use its [columnar index](https://commoncrawl.org/columnar-index) for a measured bulk discovery workload.
