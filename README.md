...
This is an independent research project inspired by a conversation about investor research at Telescope Partners. It is not affiliated with Telescope and contains no internal Telescope material.
...
Run `npm run dashboard`, then open `http://127.0.0.1:8080/`. This serves the saved exports without starting Postgres or changing evidence. The atlas opens at `/` with an explicitly labelled, bounded example neighborhood. Search or browse a dated source, focus a candidate, and select a connection to inspect its exact retained records. The research desk at `/index.html` provides record search, company studies, comparison, and source audits. Run `npm run test:dashboard` for calculation, chart, URL-state, and export-contract checks. Record selections and deeper market drill state are addressable in the URL.

The dashboard and local development server serve the same atlas and research desk. The retained-data catalog exposes all 89 snapshots with normalized text, 369 normalized SEC candidates, and 19,093 Cboe participant rows through lazy typed partitions; inventory search covers all 18,076 source rows. See the [data contract](docs/data-contract.md) for rebuild commands. Source audit loads full source/year inventory partitions on demand, including rows without a candidate tag. The underlying market field contains 1,240 eligible product/project candidates and 47,288 exact source-category-year co-listings; the atlas displays a bounded source selection. Candidate and neighbor selection opens exact retained source rows in the research desk. Five retained relationship claims across four entities form a small, explicitly scoped evidence collection. Relevant relationships appear in the inspector through explicit identity links, rather than as a separate map destination. The [architecture and extension map](docs/architecture.md) documents the modules plus database view grains, timestamps, provenance, and safe extension seams.
...
The atlas opens on unreviewed source co-listings. A co-listing is a same-category, same-year inventory observation, not a company identity or market relationship. Contextual relationship evidence and company detail expose retained claims and their source trails. The collection covers Datadog naming Elastic as a scoped competitor; dbt Labs and Snowflake announcing a partnership, a documented integration, and Snowflake's participation in dbt Labs' financing; and an untested Datadog–Snowflake shared business-driver hypothesis. These are distinct claims, even when one source supports more than one. Their source summaries, attribution, evidence locations, dates, interpretations, and remaining unknowns stay visible. Missing evidence means this slice has not recorded an eligible claim, not that no relationship exists. A recorded acceptance decision does not establish independent verification or comprehensive coverage. Map placement and line style do not measure relationship strength or performance correlation.
